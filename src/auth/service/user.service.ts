...
import { FollowRequestEntity, FriendRequestStatus } from '../entities/follower_r.entity';
...
export class UserService {
  constructor(
    ...
    switchFollow(followingId: string, followerId: string): Observable<FriendRequestStatus> {
        return from(this.followerRepository.findOne({
            where:{
    ...
    switchBlock(blockerId: string, blockedId: string): Observable<void> {
        return from(this.blockRepository.findOne({
            where: {
                blockerId: blockerId,
                blockedId: blockedId
            }
    ...
    acceptFollowRequest(senderId: string, receiverId: string): Observable<void> {
        return from(this.followRequestRepository.findOne({
            where: {
                senderId: senderId,
    ...
    getFollowRequests(userId: string): Observable<FollowRequestEntity[]> {
        return from(this.followRequestRepository.find({
            where: {
                receiverId: userId
            }
        }));
    ...
    getFollowers(userId: string): Observable<FollowerEnitity[]> {
        return from(this.followerRepository.find({
            where: {
                followingId: userId
            }
        }));
    ...
    getBlocks(userId: string): Observable<BlockEntity[]> {
        return from(this.blockRepository.find({
            where: {
                blockerId: userId
            }
        }));
